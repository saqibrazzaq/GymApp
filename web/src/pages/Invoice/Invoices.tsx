...
import { InvoiceRes } from "../../dtos/Invoice";
...
const Invoices = () => {
  ...
  const [pagedRes, setPagedRes] = useState<PagedResponse<InvoiceRes>>();
  ...
  const searchInvoices = () => {
    InvoiceApi.search(Object.fromEntries(searchParams))
      .then((res) => {
  ...
  const displayHeading = () => (
    <Flex>
      <Box>
        <Heading fontSize={"xl"}>Invoices</Heading>
      </Box>
      <Spacer />
      <Box>
        <Link as={RouteLink} to={"edit"}>
          <RegularButton text="Create Invoice" />
        </Link>
          ...
          {pagedRes?.pagedList ? (
            pagedRes.pagedList.map((item) => (
              <Tr key={item.invoiceId}>
                <Td>{item.fullName}</Td>
                <Td>{Common.formatDate(item.issueDate)}</Td>
                <Td>{item.status?.name}</Td>
                <Td>{item.amountDue}</Td>
                <Td>
                  <Link as={RouteLink} ms={2} to={item.invoiceId + "/edit"}>
                    <EditIconButton />
                  </Link>
                  <Link as={RouteLink} ms={2} to={item.invoiceId + "/delete"}>
                    <DeleteIconButton />
                  </Link>
                </Td>
              </Tr>
